...
class Runner:
  ...
  async def run_async(
    ...
    async def _run_with_trace(
      ...
      with tracer.start_as_current_span('invocation'):
        ...
        async with Aclosing(
            self._exec_with_plugin(invocation_context, session, execute, False)
        ) as agen:
          async for event in agen:
            yield event
  ...
  def _should_append_event(self, event: Event, is_live_call: bool) -> bool:
    ...
    if (
        is_live_call
        and event.author == 'model'
        and event.content
        and event.content.parts
        and event.content.parts[0].inline_data
        and event.content.parts[0].inline_data.mime_type == 'audio/pcm'
    ):
      return False
    return True
  ...
  async def run_live(
    ...
    async with Aclosing(
        self._exec_with_plugin(invocation_context, session, execute, True)
    ) as agen:
      async for event in agen:
        yield event
